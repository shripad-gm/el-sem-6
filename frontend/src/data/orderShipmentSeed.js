...
let _logisticsCache = null;

/** Single cached snapshot so orders ↔ shipments reference the same IDs */
export function getMesLogisticsSeed() {
  if (!_logisticsCache) {
    const orders = buildSeedOrders();
    _logisticsCache = { orders, shipments: buildSeedShipments(orders) };
  }
  return _logisticsCache;
}
